...
Tuple (HOT) (https://www.postgresql.org/docs/current/storage-hot.html)
updates when transfer status changes. This is critical because status
transitions (pending to completed/returned) are common operations and we
...
- 2-3x faster updates compared to non-HOT updates (https://www.razsamuel.com/understanding-hot-updates-in-postgresql-a-benchmark-analysis/)
- No table and index bloat from status changes
- Make vacuum maintenance tasks on the table simpler
- Smaller WAL file, since there is less activity overall.  (https://www.adyen.com/knowledge-hub/postgresql-hot-updates)
...
As stated on section [[*On auditing and reconstructing state][On auditing and reconstructing state]], in order to
...
VALUES ('123456789', '000000000001', 'Alice Checking');

INSERT INTO finance.external_account (routing_number, account_number, account_name)
VALUES ('987654321', '000000000002', 'Bob External');

-- 2. Create a transfer (starts as pending, period open to infinity)
INSERT INTO finance.transfer (
  routing_number, account_number,
  counterparty_routing_number, counterparty_account_number,
  amount
) VALUES (
  '123456789', '000000000001',
  '987654321', '000000000002',
  50000
);

-- 3. Create a pending transaction against the pending transfer
INSERT INTO finance.pending_transaction (
  routing_number, account_number,
  counterparty_routing_number, counterparty_account_number,
  amount,
  ...
  '987654321', '000000000002',
  50000,
  (SELECT transfer_created_at FROM finance.transfer
    WHERE routing_number = '123456789'
      AND account_number = '000000000001'
    LIMIT 1)
);

-- 4. Inspect transfer before closing
SELECT transfer_created_at, status, transfer_period, 
       lower(transfer_period) AS period_lower,
       upper(transfer_period) AS period_upper
  FROM finance.transfer;

-- 5. Close the transfer (pending -> completed)
UPDATE finance.transfer
   SET status = 'completed'
 WHERE routing_number = '123456789'
   AND account_number = '000000000001'
...
-- 6. Inspect transfer after closing --- transfer_period should now be closed,
--    but transfer_created_at (generated from lower) should be UNCHANGED
SELECT transfer_created_at, status, transfer_period,
       lower(transfer_period) AS period_lower,
       upper(transfer_period) AS period_upper
  FROM finance.transfer;

-- 7. Verify the generated column didn't change
--    transfer_created_at should equal lower(transfer_period) in both cases
SELECT transfer_created_at = lower(transfer_period) AS generated_col_consistent
  FROM finance.transfer;

-- 8. Check the status log captured the old state
SELECT * FROM finance.transfer_status_log;

-- 9. Now create a settled transaction against the completed transfer
INSERT INTO finance.settled_transaction (
  routing_number, account_number,
  counterparty_routing_number, counterparty_account_number,
  amount,
...
-- 10. Try a second transfer that gets returned instead
INSERT INTO finance.transfer (
  routing_number, account_number,
  counterparty_routing_number, counterparty_account_number,
  amount
...
-- 12. Final state of all transfers
SELECT transfer_created_at, status, transfer_period, amount
  FROM finance.transfer
 ORDER BY transfer_created_at;

-- 13. Full status history
SELECT * FROM finance.transfer_status_log
 ORDER BY sys_period;
#+end_src
