...
#+begin_src sql :tangle "only_postgres_code.sql" :comments yes
  ...
  create table if not exists finance.transfer_status_log (
    like finance.transfer including all
  );
...
#+begin_src sql :tangle "only_postgres_code.sql" :comments yes
  ...
  create table if not exists finance.managed_active_account_log (
    like finance.managed_active_account including all
  );
...
complete the transfer, and insert a settled transaction. Every
constraint trigger, the state machine, the temporal foreign key, and the
balance ledger triggers all fire on each run.
...
#+begin_src sql :tangle "pgbench_write.sql" :comments yes
  ...
  select lpad((:managed_id + 100)::text, 9, '0') as m_routing,
         lpad((:managed_id + 100)::text, 12, '0') as m_account,
         lpad(:external_id::text, 9, '0') as e_routing,
         lpad(:external_id::text, 12, '0') as e_account
  \gset
  ...
  values
    ('transfer', :'m_routing', :'m_account',
     :'e_routing', :'e_account', :amount);

  -- 2. Retrieve the transfer we just created
  select created_at as xfer_ts
    from finance.transfer_activity
   where kind = 'transfer'
     and routing_number = :'m_routing'
     and account_number = :'m_account'
     and counterparty_routing_number = :'e_routing'
     and counterparty_account_number = :'e_account'
     and status = 'pending'
   order by created_at desc
   fetch first 1 row only
  \gset

  -- 3. Insert pending transaction via the view
  insert into finance.transfer_activity
    (kind, routing_number, account_number,
     counterparty_routing_number, counterparty_account_number,
     created_at, amount)
  values
    ('pending_transaction', :'m_routing', :'m_account',
     :'e_routing', :'e_account',
     :'xfer_ts', :amount);

  -- 4. Complete the transfer via the view
  ...
  -- 5. Insert settled transaction via the view
  insert into finance.transfer_activity
    (kind, routing_number, account_number,
     counterparty_routing_number, counterparty_account_number,
     created_at, amount)
  values
    ('settled_transaction', :'m_routing', :'m_account',
     :'e_routing', :'e_account',
     :'xfer_ts', :amount);
#+end_src
...
#+begin_src sql :tangle "pgbench_read.sql" :comments yes
  ...
  select *
    from finance.transfer_activity
   where routing_number = :'m_routing'
     and account_number = :'m_account'
   order by created_at desc
...
| Step | Path through the system |
|------+-------------------------|
| Transfer insert | View ~INSTEAD OF~ trigger → ~finance.transfer~ → temporal_tables trigger → temporal FK against ~managed_active_account~ → FK against ~external_account~ |
| Pending txn insert | View trigger → ~finance.pending_transaction~ → ~within_transfer_period~ constraint → ~requires_pending_transfer~ constraint → balance ledger trigger (available only) |
| Transfer completion | View trigger → ~finance.transfer~ ~UPDATE~ → state machine trigger (closes period) → ~no_future_transactions_on_close~ constraint → temporal_tables history |
| Settled txn insert | View trigger → ~finance.settled_transaction~ → ~within_transfer_period~ constraint → ~requires_non_pending_transfer~ constraint → balance ledger trigger (current + available) |
| Read activity | View ~UNION ALL~ over 4 tables, ordered by ~created_at~ |
| Read balance | Index-backed lookup on ~balance_ledger~ |
| Serialization conflicts | Concurrent writes to same account → SSI aborts → pgbench reports as failures |

**** Interpreting the results

The pgbench output will report:

- *tps (transactions per second)*: At the startup scale of 10,000
  transfers/day (~0.12 TPS), even modest hardware should exceed this by
  orders of magnitude, confirming headroom.
- *number of failed transactions*: These are serialization failures from
  SSI. They represent cases where concurrent writes to the same account
  created a dependency cycle. In production, the application would retry
  these with exponential
  backoff[cite:@brooker15:_expon_backof_and_jitter]. The failure rate
  tells you how often retries would be needed.
- *latency (avg/stddev)*: Per-transaction latency including all trigger
  and constraint overhead. This is the true cost of enforcing all
  business rules at the data level.

A healthy result looks like: high TPS relative to the 0.12 TPS
requirement, low serialization failure rate (under 5%), and
sub-millisecond average latency for reads.
