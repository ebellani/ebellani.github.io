...
**** Accounts and temporal constraints

Financial accounts can be deactivated and reactivated. This falls neatly
within the set of temporal features introduced in SQL
2011[cite:@10.1145/2380776.2380786], in partiucular application time,
...
#+name: the_account
#+begin_src sql :tangle only_postgres_code.sql :comments link
  create table finance.active_account(
...
can change, the status and the closing of the transfer period.
...
#+begin_src sql :tangle only_postgres_code.sql :comments link
  create table finance.transfer (
    ...
    -- regular primary key: one transfer per account pair
    primary key (
      routing_number,
      account_number,
...
#+begin_src sql :tangle "only_postgres_code.sql" :comments yes
  ...
  create index on finance.transfer_status_log (routing_number, account_number, counterparty_routing_number, counterparty_account_number);
...
Transfers are the ideal scenario for HOT, since the indexed columns
via the PK do not include the column that will get updated:
~transfer_status~. The ~fillfactor = 90~ setting (already applied in the
CREATE TABLE statement above) reserves 10% of each page for updates,
allowing PostgreSQL to place updated row versions on the same page
without touching indexes.

This gives us the major benefit of not having to touch indexes,
...
[fn:11] I got the idea of such table from a similar table found on
...
[fn:14] (schemas are synonymous to models in this context)
...
[fn:17] I like the terms superordinate, coordinates and subordinates, but
they are not in common usage

[fn:18] Including a claim of cost reductions of over 90%!

[fn:19] Recently I had a conversation on linkedin: https://www.linkedin.com/feed/update/urn:li:ugcPost:7316402764975075328?commentUrn=urn%3Ali%3Acomment%3A%28ugcPost%3A7316402764975075328%2C7316457084038930432%29&dashCommentUrn=urn%3Ali%3Afsd_comment%3A%287316457084038930432%2Curn%3Ali%3AugcPost%3A7316402764975075328%29

[fn:20] From the portuguese, translated by this author.

[fn:21] Have you noticed that the second paragraph supports what the ancients did, in refeering to Plato and Aristotle?

[fn:22] DO NOT manage your partitions by hand: Use pg_partman.
