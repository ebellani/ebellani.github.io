...
transfers intermingling it with the proper transactions:

#+name: finance_transfer_history
#+begin_src sql :tangle "only_postgres_code.sql" :comments yes
  select 'transfer' as kind,
         amount,
         status,
         transfer_created_at as created_at
    from finance.transfer
   where routing_number = ?
     and account_number = ?
     and transfer_created_at = ?
  ...
  select 'transfer' as kind,
         ...
         status,
         transfer_created_at as created_at
    from finance.transfer_status_log
   where routing_number = ?
     and account_number = ?
     and transfer_created_at = ?
     and counterparty_routing_number = ?
  ...
  select 'transaction' as kind,       
    ...
    from finance.pending_transaction
   where routing_number = ?
     and account_number = ?
     and transfer_created_at = ?
     and counterparty_routing_number = ?
  ...
  select 'transaction' as kind,       
         ...
         'settled' as status,
         transaction_created_at as created_at
    from finance.settled_transaction
   where routing_number = ?
     and account_number = ?
     and transfer_created_at = ?
     and counterparty_routing_number = ?
...
#+begin_src sql :tangle "only_postgres_code.sql" :comments yes
  alter database finance set default_transaction_isolation = 'serializable';
#+end_src
...
[fn:2] A note on the code snippets: we will show code as we progress on the article, but the full code suite can be found on the [[#appendix-a-full-code-suite][Appendix A: Full code suite]]
