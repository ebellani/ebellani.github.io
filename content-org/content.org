...
#+begin_src sql :tangle only_postgres_code.sql :comments link
  ...
  create table finance.transfer (  
    ...
    -- Ensure account and counterparty are different, i.e., no transfers to itself
    check (
  ...
  grant update (status) on finance.transfer to application;
  ...
  revoke update on finance.settled_transaction from application;
...
that would necessitate ~7.5~ Gb of RAM. If we are ta talking something
like a mid/large bank, 50
