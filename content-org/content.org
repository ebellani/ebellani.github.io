...
originating transfers. Here is a requirements snippet from a very
popular banking API company:
...
#+begin_src sql :tangle only_postgres.sql :comments link
  ...
  comment on table finance.transfer_state_change is 'Represents the state changes that happen on the transfer. In particular, financial transaction are connected to such state changes.';
  ...
  create constraint trigger pending_transaction_requires_pending_transfer
    ...
    deferrable initially deferred
    for each row  
   t.transfer_created_at     = NEW.transfer_created_at
...
For the design in [[*The setup][the setup]], only /transfer/ is mutable. Everything else
is append-only. This means the entire list of temporal requirements are
achieved by maintaining a temporal history of one table.
...
#+begin_src sql :tangle "only_postgres.sql" :comments yes
  ...
  -- this will generate a public.transfer_history that will hold the history of the transfers
  -- presumes temporal_tables is installed
  call make_temporal_table('finance', 'transfer');
#+end_src
...
over time. You can query the state of a transfer by doing something like:

#+begin_src sql :tangle "only_postgres.sql" :comments yes
  select * from transfer where (transfer_created_at, transaction_account, counterparty_account) = ('2025-10-10 00:50:03.768892+00', 'acc_12', 'acc_13')
  UNION ALL
  select * from transfer_history where (transfer_created_at, transaction_account, counterparty_account) = ('2025-10-10 00:50:03.768892+00', 'acc_12', 'acc_13');

  -- ─[ RECORD 1 ]────────┬──────────────────────────────────────────────────────────────────
  -- transfer_created_at  │ 2025-10-10 00:50:03.768892+00
  -- transaction_account  │ acc_12
  -- counterparty_account │ acc_13
  -- amount               │ 4258
  -- transfer_status      │ completed
  -- sys_period           │ ["2025-12-12 20:56:14.176726+00",)
  -- ═[ RECORD 2 ]════════╪══════════════════════════════════════════════════════════════════
  -- transfer_created_at  │ 2025-10-10 00:50:03.768892+00
  -- transaction_account  │ acc_12
  -- counterparty_account │ acc_13
  ...
  -- sys_period           │ ["2025-12-12 20:56:10.909782+00","2025-12-12 20:56:14.176726+00")
#+end_src
