...
2. <<lst>>Transactions and transfers are listed, paginated by their
   respective creation times.
3. <<blc>>Current and available balance are shown, both their present and
   historical daily values
4. <<trf>>Transfers behave like a state machine where the progression
...
5. <<txn>> The user can also see the details of a transaction, and see
   the transfer that generated it.
6. <<perf>> We should maximize write throughput of transactions and
   transfers. Transfers are editable, and so we should be able to update
   them fast too.

*** Laying the foundation 

In this section we build the he core tables[fn:2] and the role
necessary to restraint updates and achieve the immutability mentioned on
the requirements (requirement [[act]] for example).

Notice in particular the partitioning of the tables. This is necessary
because we assume that our data will exceed the physical memory of the
database server, which is the core rule of thumb of such decisions
[cite:@postgresql-partitioning-docs].

# https://www.adyen.com/knowledge-hub/introduction-to-table-partioning
1) Query performance, specially for calculating the current balance or
   listing transactions of particular time ranges (see requirements [[blc]]
   and [[lst]]).
 #+begin_quote
   Query performance can be improved dramatically in certain situations,
   particularly when most of the heavily accessed rows of the table are
   in a single partition or a small number of partitions. Partitioning
   effectively substitutes for the upper tree levels of indexes, making
   it more likely that the heavily-used parts of the indexes fit in
   memory. [cite:@postgresql-partitioning-docs]
   #+end_quote

2) Stable growth: Partitioning allows us to control the estimated size
   of the underlying tables. This will prove important for write
   throughput (more on that on section [[*Partitioning for speed][Partitioning for speed]]).

#+name: laying_foundation
#+begin_src sql :tangle only_postgres_code.sql :comments link
  create schema finance;

  create role application;
  grant usage on schema finance to application;
  ...
  create domain routing_number as text
    check (value ~ '^[0-9]{9}$');
  ...
  comment on table finance.account is
    'Accounts are what transactions are performed against. Think of your bank account. They store money, receive transfers, and send payments. They earn interest and have depository insurance.';
  ...
  create table finance.transfer (
    ...
    transaction_account_number finance.account_number not null,
    transaction_routing_number finance.routing_number not null,
    ...
    primary key (
      ...
      -- connect to accounts, both the account and its counterparty
      foreign key (
  ...
  comment on table finance.transfer is 'Represents an attempt to transfer ownership of money. Since such transfer can have a rich lifecycle (approvals, reversals, etc) this entity can potentially be associated to multiple financial transactions. ';

  revoke update on finance.transfer from application;
  grant update (transfer_status_changed_at, status, status_message) 
    on finance.transfer to application;
  ...
  create table finance.settled_transaction (
    ...
    transaction_account      text not null references account(account_name),
    counterparty_account     text not null references account(account_name),  
    amount                   bigint not null, 
    foreign key (
      transaction_account,    
      transfer_created_at,    
    ...
    ) references finance.transfer_state_change (    
      transaction_account,
      transfer_created_at,    
      counterparty_account,
      transfer_status_changed_at
...
*** Maintaining business rules via meaningful constraints 
...
In the absence of SQL's ~assert~ (which is the case for
Postgres[cite:@postgresql2026unsupported] but not for
Oracle[cite:@oracle2026assertion]) we need to rely on constraint
...
#+begin_src sql :tangle only_postgres_code.sql :comments link
  create or replace function finance.is_pending_transfer(
    ...
    select exists (
      select 1
       ...
       where t.transfer_created_at     = p_transfer_created_at
         and t.transaction_account     = p_transaction_account
         and t.counterparty_account    = p_counterparty_account
         and t.transaction_created_at  = p_transaction_created_at
  ...
  begin
    if not finance.is_pending_transfer(
      ...
      NEW.transaction_account,
      NEW.counterparty_account,
      NEW.transfer_status_changed_at
    ) then
      raise exception
        'Must reference a pending transfer (% %, %)',
        NEW.transfer_created_at, NEW.transaction_account, NEW.counterparty_account;
    end if;
    return NEW;
  end;
  $$;
...
*** On planning the capacity the partition

As can be seen on the [[laying_foundation][foundationnal code block]], we have 5 core tables

1. ~finance.transfer~
2. ~finance.transfer_history~
...
| transaction_account_number     | text(12)    |          13 |
| transaction_routing_number     | text(9)     |          10 |
...
~finance.transfer_history~) + 2 (1 ~finance.pending_transaction~ + 1
~finance.settled_transaction~). Assuming 10k accounts each having 100
transfer/pending/settled per day, that would necessitate ~7.5~ Gb of
RAM. If we are ta talking something like a mid/large bank, 50
million(https://international.nubank.com.br/company/pix-breaks-record-and-reaches-1-billion-monthly-transactions/)
~38GB, and for
VISA(https://annualreport.visa.com/chairman-and-ceo-message/default.aspx#:~:text=toggle%20all-,A%20strong%202025,901%20million%20transactions%20per%20day.)
~700Gb.

ADD INDEX SIZE

*** Auditing and reconstructing state

Given the financial nature of the presumed business rules here, it
stands to reason that you'd need the classical temporal requirements:
...
- Recovering from accidental data changes and application errors
  Auditing all data changes and performing data forensics when necessary
  [cite:@microsoft24:_tempor]
#+end_quote

For the design in [[*The setup][the setup]], the changes in /transfer/ are already
capture, since they are part of the business rules. But accounts don't
have such business requirement, yet we still want to capture such
changes in the event of needing to debug or completely audit our system.

This is usually handled with the combination of several logging tools
outside of Postgres, increasing the operating cost and sometimes not
achieving the desired features. PostgreSQL on the other hand supports
several approaches for temporal auditing. For simplicity and portability
(including RDS), I use the temporal_tables extension [cite:@nearform_temporal_tables].

The extension automatically writes old versions of each row to a sibling
~_history~ table on every ~UPDATE~ or ~DELETE~. Together with a
tstzrange period, this gives you a full history suitable for ~AS OF~
queries and state reconstruction. If you install the extension in a
table with a simple function call, use this snippet:

#+begin_src sql :tangle "only_postgres.sql" :comments yes
  create or replace procedure make_temporal_table(in _schema_name text, in _table_name text)
    language plpgsql
  as $_$
    declare
      history_sql text;
  begin
    history_sql := format(
      $his$
      alter table %1$s.%2$s add column if not exists sys_period tstzrange default tstzrange(current_timestamp, null);;
      alter table %1$s.%2$s alter column sys_period set not null;
      create table if not exists %1$s.%2$s_history
      (
        like %1$s.%2$s
      );
      create index on %1$s.%2$s_history (sys_period);
      drop trigger if exists %2$s_temporal on %1$s.%2$s;
      create trigger %2$s_temporal
      before insert or update or delete
      on %1$s.%2$s
      for each row
      execute procedure versioning('sys_period', '%1$s.%2$s_history', true);
      $his$, _schema_name, _table_name);
    execute history_sql;
  end;
  $_$;

  -- this will generate a public.account_history that will hold the history of an account. 
  call make_temporal_table('finance', 'account');
#+end_src

Once applied, ~public.account_history~ will contain every past version
of every account, from which you can reconstruct the state of accounts
over time. You can query the state of all account by doing something like:
...
You can see a full tutorial on this library here [cite:@clarkdave2015_temporal_tables].

*** Write/update throughput is too low

If you want to maintain semantic enforcement of your data (which in our
model is done via fkeys and constraint triggers) about the only thing
you can do to optimize writes is to use indexes in a smart way. After
all, ~indexes, if not carefully chosen, can kill performance in a
write-heavy
...
#+begin_src sql
  ...
  ════════════╪═════════════════════╪═════════════════════════════════╪══════════╪══════════════╪═══════════════
   ...
   finance    │ transfer_history    │ transfer_history_sys_period_idx │        0 │            0 │             0
...
In the design above, there should be no updates to transactions, be them
pending or settled. Transfers on the other hand, will have their status
changed a lot. So we should benefit from making that table consist of
Heap-Only Tuples (HOT).

#+begin_quote
To allow for high concurrency, PostgreSQL uses multiversion concurrency
control (MVCC) to store rows. However, MVCC has some downsides for
update queries. Specifically, updates require new versions of rows to be
added to tables. This can also require new index entries for each
updated row, and removal of old versions of rows and their index entries
can be expensive.[cite:@postgresql_hot_2025]
#+end_quote

Transfers are the ideal scenario for HOT, since the the indexed columns
via the PK does not reach the column that will get updated:
~transfer_status~. 
...
This will give us the major benefit of not having to touch indexes,
which is the bane of write heavy
applications[cite:@gerogiannakis_postgres_index_queries_2019].

**** Partitioning for speed 

As we have seen, indexes are the bane of inserting information. Fitting
all the necessary index nodes in memory is key for fast
insertion/updates (remember that an update is also an insert).

For tables where you are using sequential identifiers, the data is
logically arranged on the right-most part of an index, avoiding page
splitting and easily keeping all necessary nodes in memory.

For tables that do not fit the above pattern, you have a problem
specially when the indexes grow to outpace available memory, which can
be the case with a high insert rate:

#+begin_quote
Unnecessary splits and rebalancing add space consumption and processing
latency to write operations. This extra IO shows up in Write Ahead Log
(WAL) generation as well.

Buildkite reported a 50% reduction in write IO for the WAL by moving to
time-ordered UUIDs.[cite:@atkinson_avoid_uuidv4_primary_keys_2025; @chan_buildkite_uuidv7_2023]
#+end_quote

We are designing here for both high insertion rates and to be able to
calculate balances fast. To calculate account balances fast, we would
have to index by ~transaction_account~, which are not aligned to the
index, obviously. The solution for that is twofold:

1. Index by transaction_account
2. Partition by a range that would fit into the cache. Let's assume that
   this would be a day. Such decision aligns neatly with a daily
   balance, which is a common feature on financial systems.

On our design, we have already created native partitioning for both
transaction tables/types. We now need to plug in all the necessary
tooling around to make sure it has the partitions underneath it and that
they are maintained over time.[fn:4]
[cite:@pg_partman_github;@aws_rds_postgresql_partitions]. Here's a
snippet that creates partitions for an year in the past and in the
future:
...
#+begin_src sql :tangle "only_postgres.sql" :comments yes
    ...
    -- set partman for a table, creating the initial partitions
    SELECT partman.create_parent(
        p_parent_table := 'finance.settled_transaction',
        p_control      := 'transaction_created_at',
        p_type         := 'range',
        p_interval     := '1 day',
        p_premake      := 7,
    ...
    -- repeat for pending transactions and transfers
    SELECT partman.create_parent(
        p_parent_table := 'finance.pending_transaction',
        p_control      := 'transaction_created_at',
        p_type         := 'range',
        p_interval     := '1 day',
        p_premake      := 7,
    ...
    CREATE TABLE partman.transfer_template (LIKE finance.transfer including all) WITH (fillfactor = 90);
    ...
    SELECT partman.create_parent(
        ...
        p_default_table := false,      
        p_start_partition := '2025-01-01 00:00:00',
        p_template_table  := 'partman.transfer_template' -- Forces partitions to use the fillfactor in the custom template 
    );

    UPDATE partman.part_config
    SET infinite_time_partitions = true,
        premake = 7                     
      WHERE parent_table in ('finance.settled_transaction', 'finance.pending_transaction', 'finance.transfer');
...
*** Appendix: Code Generating test data
:PROPERTIES:
...
#+begin_src sql :tangle "only_postgres.sql" :comments yes
  ...
  begin
    ...
    select
      ...
      from generate_series(1, days * avg_transfers_per_day) on conflict do nothing;
  ...
  begin
    ...
    from to_complete c
    where t.transfer_created_at    = c.transfer_created_at
      and t.transaction_account    = c.transaction_account
      and t.counterparty_account   = c.counterparty_account
...
#   --   create table finance.transfer_state_change (
#   --     transfer_created_at        timestamptz not null,
#   --     transaction_account        text not null references account(account_name),
#   --     counterparty_account       text not null references account(account_name),  
#   --     transfer_status_changed_at timestamptz not null,
#   --     status                     finance.transfer_status not null,
...
#   -- comment on table finance.transfer_state_change is 'Represents the state changes that happen on the transfer. Some of these changes might be tied to financial transactions, but not all. Essential to answer questions related to the state of the transfer.';


#   --   -- the app should not be able to change the transfer_state_change except
#   --   -- via interacting with the transfer table.
#   --   revoke all on finance.transfer_state_change from application;
#   --   grant select on finance.transfer_state_change to application;

#   --   create or replace function finance.log_transfer_state_change()
#   --     returns trigger language plpgsql as $$
#   --     begin    
#   --       insert into finance.transfer_state_change (
#   --         transfer_created_at,
#   --         transaction_account,
#   --         counterparty_account,
#   --         transfer_status_changed_at,
...
#   --   create trigger transfer_state_change_logger
#   --     after update
#   --     on finance.transfer
#   --     for each row
#   --     execute function finance.log_transfer_state_change();
...
#   --   -- include amount on the primary key, performance hack to calculate balances
#   --   create unique index idx_settled_transaction_pkey_covering_amount
#   --     on finance.settled_transaction (transaction_account, transaction_created_at) 
#   --     include (amount);

#   --   alter table finance.settled_transaction
#   --     add constraint settled_transaction_pkey primary key using index idx_settled_transaction_pkey_covering_amount;
...
#   --   create table finance.transfer_pending_transaction(
#   --     like finance.pending_transaction including excluding indexes,
#   --     foreign key (
#   --       transaction_account, transaction_created_at        
#   --     ) references finance.pending_transaction (    
#   --       transaction_account, transaction_created_at
#   --     ) deferrable initially deferred,
#   --     foreign key (
#   --       transaction_account,    
...
#   --       transfer_status_changed_at
#   --     ) deferrable initially deferred,
#   --     primary key(transaction_account, transfer_created_at, counterparty_account, transfer_status_changed_at, transaction_created_at)
#   --   ) partition by range (transfer_created_at);

#   --   comment on table finance.transfer_pending_transaction is 'A way to retrieve all pending transactions for a transfer quickly. See this as a sort of secondary index that connects tables that are partitioned by different data.';

#   --   create table finance.transfer_settled_transaction(
#   --     like finance.transfer_pending_transaction including all,
#   --     foreign key (
#   --       transaction_account, transaction_created_at        
#   --     ) references finance.settled_transaction (    
#   --       transaction_account, transaction_created_at
#   --     ) deferrable initially deferred,
#   --     foreign key (
#   --       transaction_account,    
...
#   --   comment on table finance.transfer_settled_transaction is 'A way to retrieve all settled transactions for a transfer quickly. See this as a sort of secondary index that connects tables that are partitioned by different data.';
...
#   --       if exists (
#   --         select 1
#   --           from finance.transfer_state_change t
#   --          where t.transfer_created_at     = NEW.transfer_created_at
#   --            and t.transaction_account     = NEW.transaction_account
#   --            and t.counterparty_account    = NEW.counterparty_account
#   --            and t.transaction_created_at  = NEW.transfer_status_changed_at
#   --            and t.transfer_status         = 'pending'
#   --       ) then
#   --         raise exception
#   --         'Settled transaction cannot reference pending transfer (% %, %)',
#   --         NEW.transfer_created_at, NEW.transaction_account, NEW.counterparty_account;
#   --       end if;
#   --       return NEW;
#   --     end;
...
#   --            and t.counterparty_account    = NEW.counterparty_account
#   --            and t.transaction_created_at  = NEW.transfer_status_changed_at             
#   --            and t.transfer_status in ('pending', 'returned')
#   --       ) then
#   --         raise exception
#   --         'Pending transaction must reference pending transfer (% %, %)',
#   --         NEW.transfer_created_at, NEW.transaction_account, NEW.counterparty_account;
#   --       end if;
#   --       return NEW;
#   --     end;
...
#+name: core_design
#+begin_src sql :tangle only_postgres.sql :comments link
  create schema finance;
  ...
  create table finance.transfer (
    ...
    transfer_status_changed_at timestamptz not null default now(),
    status                     finance.transfer_status not null,
    status_message             text not null,  
    primary key (
      transaction_account,    
...
as judged by the peer-review process. [fn:5]
#+end_quote
...
architectures[fn:8][cite:@kolny23:_scalin_prime_video;
...
To avoid that, here are 3 steps, and a reference:
   ...
   1) Ask and *embrace* feedback from bosses, peers and followers[fn:9],
...
least 2014[fn:10]. This has happened despite serious and well documented
[[#drawbacks][drawbacks]].
...
- It freezes APIs. Once a microservice establishes an API, it becomes
  ...
  API. Legacy APIs linger around, and new APIs are patched on top.[fn:11]
...
context-sensitive examples[fn:12]:
...
Conceptual schema -> Logical schema -> Physical schema [fn:12]
...
14 Themes of the Elite Private School Curriculum[fn:13]:
...
the IT sector in particular, is nominalism.[fn:14]
...
[fn:3] Such constraints are almost never created at the data level, exposing applications to the risk of failing audits and worse.

[fn:4] DO NOT manage your partitions by hand: Use pg_partman.

[fn:5] Have you noticed that the second paragraph supports what the ancients did, in refeering to Plato and Aristotle?

[fn:6] From the portuguese, translated by this author.

[fn:7] Recently I had a conversation on linkedin: https://www.linkedin.com/feed/update/urn:li:ugcPost:7316402764975075328?commentUrn=urn%3Ali%3Acomment%3A%28ugcPost%3A7316402764975075328%2C7316457084038930432%29&dashCommentUrn=urn%3Ali%3Afsd_comment%3A%287316457084038930432%2Curn%3Ali%3AugcPost%3A7316402764975075328%29

[fn:8] Including a claim of cost reductions of over 90%!

[fn:9] I like the terms superordinate, coordinates and subordinates, but
they are not in common usage

[fn:10] https://trends.google.com/trends/explore?date=all&geo=US&q=microservices

[fn:11] In a certain sense, all externally facing APIs face this. Microservices just make this problem worse by making everything ~external~ and not part of the same checkable runtime.

[fn:12] (schemas are synonymous to models in this context)

[fn:13] As listed in [[https://www.youtube.com/watch?v=IZBdv2yznmI][The Ultimate History Lesson]]

[fn:14] You will probably see me return to this topic in other posts.

[fn:15] I got the idea of such table from a similar table found on
chapter 25 of  [cite:@10.5555/861613]

[fn:16] The definitions come from [cite:@fallacies_iep], but the examples
