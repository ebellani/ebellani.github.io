...
The goal here is not to argue that specialized systems are never
appropriate, but to show that the default assumption for your data
problems should be:

*You probably don’t need anything beyond PostgreSQL, and you’re paying a real cost—financial, operational, and cognitive—every time you add one more moving part.*

For that, I'm going to use a variation of the /Drosophila melanogaster/
of the database field: [[https://en.wikipedia.org/wiki/Suppliers_and_Parts_database][the classic Supplier and Parts]]
