...
** Unjustified adoption of microservices
:PROPERTIES:
...
The microservice architecture has taken a deep hold in the collective
immagination of the software engineering comunity since 2014[fn:1]. This
has happened despite significant well documented [[*The drawbacks][drawbacks]]. I'll call
this phenomena the unjustified adoption of microservices(UAM). UAM calls
for an explanation. A sketch of such explanation is my intent here, both
of the origin of the phenomena and what it is.

UAM is a reification of abstract processes[cite:@refification_britannica], where
...
3. The desire to take advantage of the cloud computing offerings that
   started to be available at the time
4.
...
- It freezes APIs. Once a microservice establishes an API, it becomes
  ...
  API. Legacy APIs linger around, and new APIs are patched on top.
