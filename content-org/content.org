...
~A manager balances present output with creating the conditions of
future output.~
...
As one can see, this definition is the same as the one for an
investor. That is correct. A manager is an investor of company's
resources (human or otherwise). He deploys them with the expectation of
generating results in the present. If he is good, he deploys them in
such a way as to create conditions to generate *more* results in the
future.

As an illustration, a manager can usually deliver more results in the
present by burning up people. It probably will increase turnover in the
future. Is it the right choice? Only with a holistic view and sound
...
PS: A complicating factor for the manager is that he is trying to
balance a system over time, but the balancing point itself changes.
