...
From these 2 images, here is a list of functionalities that I have
extracted, which I consider to be common in financial systems like
increase:

1. <<act>>Accounts are defined by immutable routing numbers and account numbers
   and have a status that can vary.
2. <<blc>>Current and available balance are shown, both their present and
   historical daily values
3. <<txn>>Pending and settled transactions are shown, with information
   about their transfers
4. <<trf>>Transfers behave like a state machine where the progression between
   states are exposed to the user. Some of these states are linked to
   pending/settled transactions.

*** Laying the foundation 

Here are the core tables[fn:2] and the role necessary to restraint updates and achieve 
