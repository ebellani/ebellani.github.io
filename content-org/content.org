...
#+begin_src sql :tangle only_postgres_code.sql :comments link
  ...
  create domain account_number as text
    check (value ~ '^[0-9]+$');
  ...
  create table finance.transfer (
    ...
    transaction_account        text not null references finance.account(account_name),
    counterparty_account       text not null references finance.account(account_name),
...
Taking ~finance.settled_transaction~ as a reference, we have this total:

| Type        | Quantity | Size | Total size |
|-------------+----------+------+------------|
| timestamptz |        2 |    8 |            |
| text        |        2 |      |            |
| bigint      |        1 |    1 |            |
