...
#+begin_src sql
  ...
  create table finance.transfer (
    ...
    transfer_status        finance.transfer_status not null,
    primary key (
  ...
  comment on table finance.transfer is 'Represents the flow of money over its lifecyle (approvals, etc), and is therefore a mutable entity.';
...
tooling around to make sure it has the partitions underneath it.[fn:2]
...
#+begin_src sql
  ...
  --ensure the system keeps creating partitions based on time
  UPDATE partman.part_config
  SET infinite_time_partitions = true, -- Create future partitions regardless of data
      retention = '6 months',          -- Keep only 6 months of active data
      retention_keep_table = true,     -- Detach instead of drop for safe archiving
      premake = 7                      -- Keep 7 days of future partitions ready
  WHERE parent_table = 'finance.settled_transaction';
  ...
  SET infinite_time_partitions = true,
      ...
      premake = 7                     
  WHERE parent_table = 'finance.pending_transaction';

  -- repeat for pending transactions
  SELECT partman.create_parent(
  ...
  SET infinite_time_partitions = true,
      ...
      retention_keep_table = true,    
      premake = 7                     
  WHERE parent_table = 'finance.transfer';

  -- run maintenance every hour
   rtman SCHEMA partman;
...
#+begin_src sql
  ...
  begin
    ...
    from finance.transfer t  
    where upper(t.sys_period) is null
      and t.transfer_status = 'completed';  -- ensure we use the new version
    return;
  end;
  $$;
#+end_src
...
[fn:2] DO NOT manage your partitions by hand: Use pg_partman [cite:@pg_partman_github;@aws_rds_postgresql_partitions]. Here's a snippet that creates partitions for an year in the past and in the future:
