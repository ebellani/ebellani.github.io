...
#+begin_src sql :tangle "only_postgres.sql :comments link
  ...
  create table finance.transfer (
    ...
    primary key (
      -- match the query pattern (give me all transfers for this account for today)
      transaction_account,    
      transfer_created_at,    
  ...
  create or replace constraint trigger settled_transaction_requires_non_pending_transfer
    after insert on finance.setttled_transaction
    deferrable initially deferred
    for each row  
  ...
  create or replace constraint trigger pending_transaction_requires_pending_transfer
