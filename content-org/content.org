...
#+begin_src sql
  ...
  create domain transfer_status as text
  ...
  comment on table transfer is 'Represents the flow of money over its lifecyle (approvals, etc), and is therefore a mutable entity.';

  create table settled_transaction (
  ...
  comment on table settled_transaction is
  ...
  create constraint trigger settled_transaction_requires_non_pending_transfer
    after insert on setttled_transaction
    deferrable initially deferred
    for each row  
    execute function ensure_non_pending_transfer();

  create or replace function ensure_pending_transfer()
    returns trigger language plpgsql as $$
    begin
      if not exists (
        select 1
  ...
  create constraint trigger pending_transaction_requires_pending_transfer
    after insert on pending_transaction
    deferrable initially deferred
    for each row  
    execute function ensure_pending_transfer();
...
_history table on every ~UPDATE~ or ~DELETE~. Together with a tstzrange
period, this gives you a full history suitable for ~AS OF~ queries and
state reconstruction. If you install the extension in a table with a simple
function call, use this snippet:
...
#+begin_src sql
  ...
  call make_temporal_table('public', 'transfer');
#+end_src
...
#+begin_src sql
  ...
  create or replace function generate_pending_activity(
    accounts_count int,
    days int,
  ...
  create or replace function complete_pending_activity(
  ...
  begin
    ...
    ---------------------------------------------------------------------
    insert into settled_transaction (
      transaction_created_at,
      transfer_created_at,
      transaction_account,
      counterparty_account,    
