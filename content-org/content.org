...
#+begin_src sql :tangle only_postgres_code.sql :comments link
  ...
  /*
  SELECT
      pg_size_pretty(pg_relation_size(c.oid) / greatest(t.n_live_tup, 1)) AS avg_row_size,
      pg_size_pretty(pg_relation_size(c.oid)) AS table_size,
      t.n_live_tup AS live_rows
  FROM
      pg_class c
  JOIN
      pg_stat_user_tables t ON c.oid = t.relid
  WHERE
      t.relid = 'finance.transfer'::regclass;
   ,*/
...
Notice the constraint triggers: These enforce crucial business rules
(pending transactions are associated with a transfer in the
pending/returned state and settled transactions are never associated
with pending transfers).

Such constraints are almost never created at the data level, exposing
applications to the risk of failing audits and worse.

*** On planning the capacity the partition

As can be seen on the [[laying_foundation][foundationnal code block]], we have 5 core
tables
1. ~finance.transfer~
...
they are maintained over time.[fn:3]
...
as judged by the peer-review process. [fn:4]
#+end_quote
...
architectures[fn:7][cite:@kolny23:_scalin_prime_video;
...
To avoid that, here are 3 steps, and a reference:
   ...
   1) Ask and *embrace* feedback from bosses, peers and followers[fn:8],
...
least 2014[fn:9]. This has happened despite serious and well documented
[[#drawbacks][drawbacks]].
...
- It freezes APIs. Once a microservice establishes an API, it becomes
  ...
  API. Legacy APIs linger around, and new APIs are patched on top.[fn:10]
...
context-sensitive examples[fn:11]:
...
Conceptual schema -> Logical schema -> Physical schema [fn:11]
...
14 Themes of the Elite Private School Curriculum[fn:12]:
...
the IT sector in particular, is nominalism.[fn:13]
...
[fn:3] DO NOT manage your partitions by hand: Use pg_partman.

[fn:4] Have you noticed that the second paragraph supports what the ancients did, in refeering to Plato and Aristotle?

[fn:5] From the portuguese, translated by this author.

[fn:6] Recently I had a conversation on linkedin: https://www.linkedin.com/feed/update/urn:li:ugcPost:7316402764975075328?commentUrn=urn%3Ali%3Acomment%3A%28ugcPost%3A7316402764975075328%2C7316457084038930432%29&dashCommentUrn=urn%3Ali%3Afsd_comment%3A%287316457084038930432%2Curn%3Ali%3AugcPost%3A7316402764975075328%29

[fn:7] Including a claim of cost reductions of over 90%!

[fn:8] I like the terms superordinate, coordinates and subordinates, but
they are not in common usage

[fn:9] https://trends.google.com/trends/explore?date=all&geo=US&q=microservices

[fn:10] In a certain sense, all externally facing APIs face this. Microservices just make this problem worse by making everything ~external~ and not part of the same checkable runtime.

[fn:11] (schemas are synonymous to models in this context)

[fn:12] As listed in [[https://www.youtube.com/watch?v=IZBdv2yznmI][The Ultimate History Lesson]]

[fn:13] You will probably see me return to this topic in other posts.

[fn:14] I got the idea of such table from a similar table found on
chapter 25 of  [cite:@10.5555/861613]

[fn:15] The definitions come from [cite:@fallacies_iep], but the examples
