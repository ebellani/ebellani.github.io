...
#+begin_src sql
  ...
  create domain transfer_status as text
    check(value in ('pending_approval',
                    'pending_submission',
  	          'canceled',
  		  'pending_reviewing',
  		  'rejected',
  ...
  create domain transaction_type as text
    check(value in ('pending', 'completed', 'settled'));
  ...
  comment on table transaction is
    'Implements both regular and pending transactions. These are immutable events, so no one should have update permissions.';

  comment on table transfer is 'Represents the flow of money, and is therefore a mutable entity.';
#+end_src

*** Auditing and reconstructing state

Given the financial nature of the presumed business rules here, it
stands to reason that you'd need:
...
This is usually handled with the combination of several tools outside of
Postgres, increasing the operating cost and sometimes not achieving the
desired features. Achieving this in PostgreSQL is straightforward with
several extension, one that is ready to use with RDS is
[cite:@nearform_temporal_tables]. Simply run that SQL on your
instance. If you want a way to drive the extension with a simple
...
#+begin_src sql
  ...
  -- this will generate a public.my_table_history that will hold the history of the my_table
  -- call make_temporal_table('pblic', 'my_table_history');
#+end_src
