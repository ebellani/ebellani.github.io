...
- It does not have observability
...
#+begin_src sql
  ...
  create table finance.pending_transaction (
    like settled_transaction including all,
    foreign key (
      transfer_created_at,
      transaction_account,
      counterparty_account
...
Say that all your indexes are necessary, but you are still facing write
problems. Here's a concrete example, say each of your tables
(transaction, pending and transfer) are of ~1TB, indexes included. Such
indexes would typically not fit memory and because of that every ~INSERT~
operation undergoes a dramatic performance shift from memory-speed
updates to disk-bound operations (a shared read).
...
as judged by the peer-review process. [fn:2]
#+end_quote
...
architectures[fn:5][cite:@kolny23:_scalin_prime_video;
...
To avoid that, here are 3 steps, and a reference:
   ...
   1) Ask and *embrace* feedback from bosses, peers and followers[fn:6],
...
least 2014[fn:7]. This has happened despite serious and well documented
[[#drawbacks][drawbacks]].
...
- It freezes APIs. Once a microservice establishes an API, it becomes
  ...
  API. Legacy APIs linger around, and new APIs are patched on top.[fn:8]
...
context-sensitive examples[fn:9]:
...
Conceptual schema -> Logical schema -> Physical schema [fn:9]
...
14 Themes of the Elite Private School Curriculum[fn:10]:
...
the IT sector in particular, is nominalism.[fn:11]
...
[fn:2] Have you noticed that the second paragraph supports what the ancients did, in refeering to Plato and Aristotle?

[fn:3] From the portuguese, translated by this author.

[fn:4] Recently I had a conversation on linkedin: https://www.linkedin.com/feed/update/urn:li:ugcPost:7316402764975075328?commentUrn=urn%3Ali%3Acomment%3A%28ugcPost%3A7316402764975075328%2C7316457084038930432%29&dashCommentUrn=urn%3Ali%3Afsd_comment%3A%287316457084038930432%2Curn%3Ali%3AugcPost%3A7316402764975075328%29

[fn:5] Including a claim of cost reductions of over 90%!

[fn:6] I like the terms superordinate, coordinates and subordinates, but
they are not in common usage

[fn:7] https://trends.google.com/trends/explore?date=all&geo=US&q=microservices

[fn:8] In a certain sense, all externally facing APIs face this. Microservices just make this problem worse by making everything ~external~ and not part of the same checkable runtime.

[fn:9] (schemas are synonymous to models in this context)

[fn:10] As listed in [[https://www.youtube.com/watch?v=IZBdv2yznmI][The Ultimate History Lesson]]

[fn:11] You will probably see me return to this topic in other posts.

[fn:12] I got the idea of such table from a similar table found on
chapter 25 of  [cite:@10.5555/861613]

[fn:13] The definitions come from [cite:@fallacies_iep], but the examples
