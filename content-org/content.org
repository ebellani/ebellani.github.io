...
First, let's create the logical schema that will hold the database
design, and the user[fn:3] and the initial domains that will provide the
values we will be using and an auxiliary table for accounts:

#+name: laying_foundation_domain
...
#+begin_src sql :tangle only_postgres_code.sql :comments link
  create table finance.transfer (
    ...
    -- temporal primary key: prevents overlapping transfers for same account pair
    primary key (
      account_number,
      routing_number,
      counterparty_account_number,
      counterparty_routing_number,
      transfer_valid_period without overlaps
    ),
    -- temporal foreign key: ensure counterparty account exists during transfer period
    foreign key (
      counterparty_account_number,
      counterparty_routing_number,
      period transfer_valid_period
  ...
  ) partition by range (lower(transfer_valid_period));
...
In the absence of SQL's ~assert~[fn:5] we need to rely on constraint
...
tables[fn:7]. PostgreSQL on the other hand supports
...
Once applied, ~public.transfer_history~ will contain every past version
of every transfer, from which you can reconstruct the state of transfers
over time. You can query the state of all transfers by doing something
like[fn:21]:
...
transaction can only reference a transfer that existed at the timethe
transaction was created.


This works through the PERIOD feature and WITHOUT OVERLAPS
clause.
...
you can do to optimize write throughpu is to lower the write
amplication, specifically to use indexes in a smart way. After all,
...
Paritioning becomes relevant because fitting all the necessary index
nodes in memory is key for fast insertion/updates (remember that an
update is also an insert).

For tables where you are using sequential identifiers and are only
inserting, you have good performance *because* you have good index
locality. Poor index locality leads to space consumption and processing
latency to write
operations[cite:@atkinson_avoid_uuidv4_primary_keys_2025]. Fixing such
issues can lead to surprising improvements such as a ~50% reduction in
the WAL (Write Ahead Log) rate of the primary
database~[cite:@chan_buildkite_uuidv7_2023].

As we can see in the [[full_list][feature list]], we have 2 features represented as
relations: [[laying_foundation_table_transfer][transfers]] and [[laying_foundation_table_txn][transactions]]. Both are sequentially ordered by
their creation time but, crucially, transactions do not change while
transfers do. *Therefore*, we should partition the transfers table to
minimize index locality problems, but not the transaction tables.

Here is the code to add partitions using pg_partman [cite:@pg_partman_github]:
...
as judged by the peer-review process. [fn:9]
#+end_quote
...
architectures[fn:12][cite:@kolny23:_scalin_prime_video;
...
To avoid that, here are 3 steps, and a reference:
   ...
   1) Ask and *embrace* feedback from bosses, peers and followers[fn:13],
...
least 2014[fn:14]. This has happened despite serious and well documented
[[#drawbacks][drawbacks]].
...
- It freezes APIs. Once a microservice establishes an API, it becomes
  ...
  API. Legacy APIs linger around, and new APIs are patched on top.[fn:15]
...
context-sensitive examples[fn:16]:
...
Conceptual schema -> Logical schema -> Physical schema [fn:16]
...
14 Themes of the Elite Private School Curriculum[fn:17]:
...
the IT sector in particular, is nominalism.[fn:18]
...
* Footnotes
[fn:21] You can see a full tutorial on this library on  [cite:@clarkdave2015_temporal_tables] 
...
[fn:8] DO NOT manage your partitions by hand: Use pg_partman.

[fn:9] Have you noticed that the second paragraph supports what the ancients did, in refeering to Plato and Aristotle?

[fn:10] From the portuguese, translated by this author.

[fn:11] Recently I had a conversation on linkedin: https://www.linkedin.com/feed/update/urn:li:ugcPost:7316402764975075328?commentUrn=urn%3Ali%3Acomment%3A%28ugcPost%3A7316402764975075328%2C7316457084038930432%29&dashCommentUrn=urn%3Ali%3Afsd_comment%3A%287316457084038930432%2Curn%3Ali%3AugcPost%3A7316402764975075328%29

[fn:12] Including a claim of cost reductions of over 90%!

[fn:13] I like the terms superordinate, coordinates and subordinates, but
they are not in common usage

[fn:14] https://trends.google.com/trends/explore?date=all&geo=US&q=microservices

[fn:15] In a certain sense, all externally facing APIs face this. Microservices just make this problem worse by making everything ~external~ and not part of the same checkable runtime.

[fn:16] (schemas are synonymous to models in this context)

[fn:17] As listed in [[https://www.youtube.com/watch?v=IZBdv2yznmI][The Ultimate History Lesson]]

[fn:18] You will probably see me return to this topic in other posts.

[fn:19] I got the idea of such table from a similar table found on
chapter 25 of  [cite:@10.5555/861613]

[fn:20] The definitions come from [cite:@fallacies_iep], but the examples
