...
database server, which is a core rule of thumb of such decisions
[cite:@postgresql-partitioning-docs]. Partiioning allows us to have:
...
2) Stable growth, allowing us to control the estimated size of the
   underlying tables. This will prove important for write throughput
   (more on that on section [[*Partitioning for speed][Partitioning for speed]]).
...
In order to implement feature [[trf]], need system-time temporal
tables[fn:4]. PostgreSQL on the other hand supports
several approaches for temporal tables. For simplicity and portability
(including RDS), we use the temporal_tables extension
[cite:@nearform_temporal_tables].
...
#+begin_src sql :tangle "only_postgres.sql" :comments yes
  ...
  create table if not exists finance.transfer_history (like finance.transfer including all);
  comment on table finance.transfer_history is 'the state history of the transfer. Useful for auditing and for features showing such state to the user';

  create index on finance.transfer_history (sys_period);

  create trigger transfer_save_history
    before insert or update or delete
    on finance.transfer
    for each row
    execute procedure versioning('sys_period', 'finance.transfer_history', true);
#+end_src
...
they are maintained over time.[fn:5]
...
as judged by the peer-review process. [fn:6]
#+end_quote
...
To avoid that, here are 3 steps, and a reference:
   ...
   1) Ask and *embrace* feedback from bosses, peers and followers[fn:10],
...
least 2014[fn:11]. This has happened despite serious and well documented
[[#drawbacks][drawbacks]].
...
- It freezes APIs. Once a microservice establishes an API, it becomes
  ...
  API. Legacy APIs linger around, and new APIs are patched on top.[fn:12]
...
context-sensitive examples[fn:13]:
...
Conceptual schema -> Logical schema -> Physical schema [fn:13]
...
14 Themes of the Elite Private School Curriculum[fn:14]:
...
the IT sector in particular, is nominalism.[fn:15]
...
[fn:4] Temporal tables can be used for several interesting features:
      #+begin_quote
...
[fn:5] DO NOT manage your partitions by hand: Use pg_partman.

[fn:6] Have you noticed that the second paragraph supports what the ancients did, in refeering to Plato and Aristotle?

[fn:7] From the portuguese, translated by this author.

[fn:8] Recently I had a conversation on linkedin: https://www.linkedin.com/feed/update/urn:li:ugcPost:7316402764975075328?commentUrn=urn%3Ali%3Acomment%3A%28ugcPost%3A7316402764975075328%2C7316457084038930432%29&dashCommentUrn=urn%3Ali%3Afsd_comment%3A%287316457084038930432%2Curn%3Ali%3AugcPost%3A7316402764975075328%29

[fn:9] Including a claim of cost reductions of over 90%!

[fn:10] I like the terms superordinate, coordinates and subordinates, but
they are not in common usage

[fn:11] https://trends.google.com/trends/explore?date=all&geo=US&q=microservices

[fn:12] In a certain sense, all externally facing APIs face this. Microservices just make this problem worse by making everything ~external~ and not part of the same checkable runtime.

[fn:13] (schemas are synonymous to models in this context)

[fn:14] As listed in [[https://www.youtube.com/watch?v=IZBdv2yznmI][The Ultimate History Lesson]]

[fn:15] You will probably see me return to this topic in other posts.

[fn:16] I got the idea of such table from a similar table found on
chapter 25 of  [cite:@10.5555/861613]

[fn:17] The definitions come from [cite:@fallacies_iep], but the examples
